...
data = urlencode({'a': 'A'})
headers = {
    'User-Agent': 'Maxim_webserver_test',
    'Accept': 'text/html',
...
servers={"s2":"93.84.119.237","s4":"93.84.119.239","s5":"93.84.119.240","s6":"93.84.119.241","s7":"93.84.119.242","s8":"93.84.119.243"}
d = dict()
err = dict()
    ...
    while True:
        ...
        if last_check_ping == "Network Active":
            for number in servers.keys():
                try:
                    server = HTTPConnection('%s.open.by' %(number), timeout=60)
            ...
            for number in servers:
                ...
                if (error_result[0][0]) != (err.get(number)):
                    ...
                    bot_err_sent(number,servers.get(number),error,st)
                    #err_mail_send(number,error,st)
                elif ((int(base_result[0][0])) != (d.get(number))) and (d.get(number) != "1"):
                    st = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
                    code=d.get(number)
                    db_worker = SQLighter(config.database)
